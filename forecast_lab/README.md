...
第二轮改进和服务器操作见 [EXPERIMENTS_V2.md](EXPERIMENTS_V2.md)：残差预测、近端加权、编码宽度对照、多种子比较和探针重复抽样。

交付状态：按要求**未在本机运行 Python、训练、测试或图表渲染**。仓库已保留服务器返回的第一轮 NREL 结果；新代码尚无实测结果。以下服务器检查命令不代表已通过。
...
- 默认直接预测模式的预测头只接收 Z；可选 `--prediction-mode residual` 将网络输出作为最后观测值上的修正量，此时存在显式最后值旁路，诊断也分别检查两条路径。
...
- `L_prediction`：有观测目标上的标准化 MSE。默认各步等权；`--near-weight` 可提高前 `--near-steps` 步的权重，早停仍读取未加权验证 MSE。
...
重新评估重用训练缩放参数，不重新拟合，并检查列结构、划分长度和首尾时间。新检查点额外使用 SHA-256 校验原始 CSV 内容；旧检查点没有该指纹，无法据此验证原始数据版本。测试集诊断仅用于事后分析，不用于调参。
